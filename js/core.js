class DNDVoiceCore {
    ...
    async init() {
        ...
        const modules = [
            ...
            // ['voiceTestSuite', './voice-test-suite.js'], // MOVED TO SETTINGS ONLY
            ...
            // VERSION CONTROL DISABLED FOR GAMEPLAY - Only enable for development
    ...
    async loadModule(name, path) {
        try {
            // Ensure path is relative to the JS directory
            const fullPath = path.startsWith('./') ? `/js/${path.slice(2)}` : path;
